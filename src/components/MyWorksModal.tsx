...
import { X, Image as ImageIcon, Heart, Bookmark, Eye, CalendarDays } from 'lucide-react';
...
export default function MyWorksModal({ open, onClose, posts, currentUser, onViewPost }: MyWorksModalProps) {
  const myPosts = useMemo(() => {
    if (!currentUser) return [];
    return [...posts]
      .filter((post) => post.artist.id === currentUser.id)
        ...
        >
          ...
          >
            <div className="p-5 border-b border-gray-100 flex items-center justify-between">
              <div>
                <h2 className="text-lg font-bold text-gray-900">Tác phẩm của tôi</h2>
                <p className="text-sm text-gray-500 mt-0.5">
                  {currentUser ? `@${currentUser.username} • ${myPosts.length} tác phẩm` : 'Bạn chưa đăng nhập'}
                </p>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="p-5">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-5">
                <div className="rounded-2xl border border-gray-100 bg-gray-50 px-3 py-3">
                  <p className="text-xs text-gray-400">Tổng tác phẩm</p>
                  <p className="text-lg font-bold text-gray-900 mt-0.5">{formatNumber(myPosts.length)}</p>
                </div>
                <div className="rounded-2xl border border-gray-100 bg-gray-50 px-3 py-3">
              ...
              <p className="text-xs font-semibold text-gray-500 mb-2">Danh sách tác phẩm</p>
              {myPosts.length > 0 ? (
                ...
                <div className="rounded-2xl border border-dashed border-gray-200 bg-gray-50 p-6 text-center">
                  ...
                  <p className="text-xs text-gray-400 mt-1">Hãy bấm nút "Đăng tải" để chia sẻ tác phẩm đầu tiên.</p>
                </div>
              )}
            </div>
          </motion.div>
